import { Bind, Controller, Delete, Body, Get,Patch,Param, HttpException,HttpStatus, Post, Query, UploadedFile, UseInterceptors } from '@nestjs/common';
  ...
  async uploadFile(file) 
  {
    ...
    if (extantion == 'pdf' || extantion == 'MP4' || extantion == 'mp3' )
      { 
        if (extantion == 'MP4')
          {
            await fsPromises.writeFile('uploads/video/'  + file.originalname,file.buffer)
            console.log("video")

            const localVideoPath = 'uploads/video/' + file.originalname;
            videoDuration(localVideoPath).then(duration => {
            ...
            await fsPromises.writeFile('uploads/pdf/'  + file.originalname,file.buffer)
      ...
      throw new HttpException('Недопустимый тип файла. Пожалуйста, загрузите файл с допустимым расширением (pdf/mp4/mp3)',HttpStatus.NOT_ACCEPTABLE)
    } 
